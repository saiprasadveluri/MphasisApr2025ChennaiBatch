namespace TravelFormsEF
{
    public partial class Form1 : Form
    {
        ...
        private void searchEngineToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            search.ShowDialog();
            // List<ServiceSearch> ser = dataAccess.GetServicesBasedOnLocation();
        }
        ...
        private void editToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (GridLocations.SelectedRows.Count > 0)
            {
                var editrow = GridLocations.SelectedRows[0];
                int locId = int.Parse(editrow.Cells[0].Value.ToString());
              
                EditLocation editLocation = new EditLocation(locId);
                editLocation.ShowDialog();
                var llist = dataAccess.GetAllLocations();
                GridLocations.DataSource = null;
                GridLocations.DataSource = llist;
                GridLocations.Refresh();
                var serlist = dataAccess.GetAllServicesView();
                GridServices.DataSource = null;
                GridServices.DataSource = serlist;
