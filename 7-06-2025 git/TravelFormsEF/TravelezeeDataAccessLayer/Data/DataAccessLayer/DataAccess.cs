...
public class DataAccess{
    ...
    public List<ServiceEntry> GetServicesBasedOnLocation(long SrcLoc,long DestLoc){
        var  res=(from srvObj in context.services
            join srvtypeobj in context.servicetypes on srvObj.ServiceTypeId equals srvtypeobj.STypeId
            where srvObj.SourceLocId==SrcLoc && srvObj.DestLocId==DestLoc
            select new ServiceEntry()
            {
                 ServiceId=srvObj.ServiceId,
                 Distance =srvObj.Distance,
                 ServiceTypeName=srvtypeobj.ServiceTypeName,
                 RatePerKm=srvtypeobj.PricePerKm,
                 TotalCharge= srvObj.Distance* srvtypeobj.PricePerKm
            }).ToList();
        return res;
...
public bool AddService(long sTypeId, long src, long dest, double distance)
    ...
    public bool EditLocation(long locationId,string locname)
    {
        var r=context.locations.FirstOrDefault(l=>l.LocationId==locationId);
        if(r != null)
        {
