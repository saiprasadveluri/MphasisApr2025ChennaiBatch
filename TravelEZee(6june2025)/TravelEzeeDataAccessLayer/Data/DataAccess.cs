...
public class DataAccess
{
    ...
    {
        Location curLoc = context.Loactions.FirstOrDefault(l =>l. LocationId == locid);
        if (curLoc != null)
        {
    ...
    public bool DeleteServices(long Servid)
    {
        var ServicesList = context.Services.FirstOrDefault(s=> s.ServiceId== Servid);
        context.Services.Remove(ServicesList);
        int del =context.SaveChanges();
        return del > 0;

     }
    public bool DeleteServicesType(long SerStypeId)
    {
        var ServicesList = context.Services.Where(st => st.ServiceTypeId == SerStypeId).ToList();
        context.Services.RemoveRange(ServicesList);
