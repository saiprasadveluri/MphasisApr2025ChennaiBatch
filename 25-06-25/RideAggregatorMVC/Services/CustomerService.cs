...
namespace RideAggregatorMVC.Services
{
    public class CustomerService
    {
        ...
        {
            var response = await _http.GetAsync("https://localhost:7155/api/Customer");
        ...
        {
            var response = await _http.PostAsJsonAsync("https://localhost:7155/api/Customer", customer);
            response.EnsureSuccessStatusCode();
        }
    }
}
