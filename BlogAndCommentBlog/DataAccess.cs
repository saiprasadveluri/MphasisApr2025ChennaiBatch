...
namespace ADDNetBlogWinApp
{
    ...
    {
        private const string ConString = "Data Source=WKSCHE03TRNG087\\SQLEXPRESS;Initial Catalog=BlogAppDB;Integrated Security=SSPI;Trust Server Certificate=True";
        ...
        public bool AddComment(string Title, string CommentText, string CommentBy,int selid)
        {
            ...
            string cmdText = $"insert into BlogComment(PostId,Title,CommentText,CommentBy) values('{selid}','{Title}','{CommentText}','{CommentBy}')";
            SqlCommand sqlCommand = new SqlCommand();
            sqlCommand.CommandText = cmdText;
            sqlCommand.Connection = _connection;
        ...
        public bool AddPost(string title,string ptext,string pby)
        {
            ...
            string cmdText = $"insert into BlogPost(title,posttext,postedby) values('{title}','{ptext}','{pby}')";
            SqlCommand sqlCommand = new SqlCommand();
            sqlCommand.CommandText = cmdText;
            sqlCommand.Connection = _connection;
            int nRecordsEffected=sqlCommand.ExecuteNonQuery();
