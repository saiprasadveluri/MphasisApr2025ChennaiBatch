...
namespace BookMyShow.Controllers
{
    ...
    public class BookMyShowController : Controller
    {
        ...
        public async Task<IActionResult> Book(int showId, int theaterId, string date, string[] selectedSeats)
        {
            ...
            try
            {
                ...
                var ticket = new Ticket
                {
                    ...
                    //SeatId = (int)db.Seat.Find(seatId),

                };
                var ticketPrice = await db.Theaters.Where(t => t.Tid == theaterId).Select(t => t.Price).FirstOrDefaultAsync();

                db.Tickets.Add(ticket);
                await db.SaveChangesAsync();
                await transaction.CommitAsync();

                TempData["Message"] = $"Seats {string.Join(", ", seatsToBook)} Selected Successfully!";
                TempData["SeatCount"] = seatsToBook.Length;
                TempData["TotalAmount"] = ((ticketPrice ?? 0) * seatsToBook.Length).ToString("F2"); // ✅ FIXED
                TempData["SeatNumbers"] = string.Join(",", seatsToBook);
        ...
        [HttpGet]
        public IActionResult Payment(int showId, int theaterId, string date,string SeatNumbers)
        {
...
;            Console.WriteLine("payment", showId, theaterId, date);
            ...
            if (tickets == null || string.IsNullOrEmpty(SeatNumbers))
            {
            ...
            var seatNumbers = SeatNumbers.Split(",");
            ...
            ViewBag.MovieId = tickets.MovieId;
            ViewBag.TheaterId = theaterId;
            ViewBag.Ticketid = tickets.Ticketid;
            ViewBag.ShowId = showId;
        ...
        public async Task<IActionResult> Payment(AllPayments model)
        {
            ...
            try
            {
                ...
                var show = await db.ShowTimes.FindAsync(model.showId);
                if (show == null)
                {
                ...
                using var transaction = await db.Database.BeginTransactionAsync();
                model.SeatNumber = model.SeatNumber.Replace("Seat", "");
                try
                {
                    var payment = new Payment
                    {
                        ...
                        SeatNumber = string.Join(",", model.SeatNumber)
        ...
        [Authorize] 
        [ResponseCache(NoStore = true, Location = ResponseCacheLocation.None)]//added1234
        public IActionResult LogOut()
        {
            HttpContext.Session.Clear();
            HttpContext.SignOutAsync();//add
            return RedirectToAction("Login", "BookMyShow");//ADDED
        }


        //fOR LOGOUT TO REMOVE BROWSER DATA
        //[Authorize]
        //[HttpPost]
