...
namespace QuizMVC.ClientServices
{
    public class ServicesMVC
    {
        ...
        public async Task<bool> AddQuesionWithOption(QuestionOptionsDTO questionoptions)
        {
            var response = await httpClient.PostAsJsonAsync("Admin/question", questionoptions);
            if (!response.IsSuccessStatusCode)
                return false;
            string result = await response.Content.ReadAsStringAsync();
            bool.TryParse (result, out bool success);
        ...
        public async Task<List<CorrectAnswerDTO>> GetCorrectAnswersForQuestions(List<Guid> questionIds)
        {
            var correctAnswers = new List<CorrectAnswerDTO>();

            foreach (var questionId in questionIds)
            {
                var response = await httpClient.GetAsync($"User/GetOptionsByQuestionId?id={questionId}");
                response.EnsureSuccessStatusCode();

                var json = await response.Content.ReadAsStringAsync();
                var question = JsonSerializer.Deserialize<GetQuestionWithOptionsDTO>(json);

                var correctOption = question.options.FirstOrDefault(o => o.answer == 1);
        ...
        public async Task<bool> SaveAttempt(QuizAttemptDTO attemptDto)
        {
            HttpResponseMessage response = await httpClient.PostAsJsonAsync("User/SaveQuizAttempt", attemptDto);
            string result = await response.Content.ReadAsStringAsync();
